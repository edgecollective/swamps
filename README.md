...
void setup() {
  ...
  fona.setGPRSNetworkSettings(F("epc.tmobile.com"));
...
void loop() {
  ...
  switch (command) {
    ...
    char* url="https://wolfesneck.farmos.net/farm/sensor/listener/5b5fc33d52a2289cfc045db0e23656a3?private_key=16c02c29a2f9d6d458934442e072e0fe";
    
    sprintf(data,"{\"value\":%d}", 23);
    Serial.println(url);
    Serial.println(data);
    int result=fona.HTTP_POST_start(url, F("application/json"), (uint8_t *) data, strlen(data), &statuscode, (uint16_t *)&length);
...
void Send2ubidots(String value)
{
  ...
  delay(3000);
  fona.sendCheckReply(F("at+cipcsgp=1,\"epc.tmobile.com\",\"user\",\"pass\""), F("OK"));  //your APN goes here
  delay(3000);

  Serial.print(F("Start the connection to Ubidots: "));
  if (SendATCommand("AT+CIPSTART=\"tcp\",\"https://wolfesneck.farmos.net/farm/sensor/listener/5b5fc33d52a2289cfc045db0e23656a3?private_key=16c02c29a2f9d6d458934442e072e0fe\",\"80\"", 'C', 'T')) {
  ...
  fona.println("POST /api/v1.6/variables/xxxxxxxxxxx/values HTTP/1.1");           // Replace "xxx..." with your variable ID
  fona.println("Content-Type: application/json");
  fona.println("Content-Length: " + le);
  //fona.println("X-Auth-Token: xxxxxxxxxxxxxxxxxxxx");                                    // here you should replace "xxx..." with your Ubidots Token
  fona.println("Host: https://wolfesneck.farmos.net/farm/sensor/listener/5b5fc33d52a2289cfc045db0e23656a3?private_key=16c02c29a2f9d6d458934442e072e0fe");
  fona.println();
  fona.println(var);
  fona.println();
